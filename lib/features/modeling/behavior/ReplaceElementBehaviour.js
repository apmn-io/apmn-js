 ...
 * that morphs into another APMN element
 */
export default function ReplaceElementBehaviour(
  ...
  this.postExecuted([ 'elements.move' ], 500, function(event) {
    ...
    // Change target to host when the moving element is a `apmn:BoundaryEvent`
...
ReplaceElementBehaviour.prototype.replaceElements = function(elements, newElements, newHost) {
  ...
  forEach(newElements, function(replacement) {
    ...
    if (newHost && is(oldElement, 'apmn:BoundaryEvent')) {
    ...
    if (newHost && is(elements[idx], 'apmn:BoundaryEvent')) {
