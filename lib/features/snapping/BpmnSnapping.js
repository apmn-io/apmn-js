...
/**
 * APMN specific snapping functionality
 *
...
export default function BpmnSnapping(eventBus, canvas, bpmnRules, elementRegistry) {
  ...
  eventBus.on('create.start', function(event) {
    ...
    if (is(shape, 'apmn:Participant') && is(rootElement, 'apmn:Process')) {
  ...
  eventBus.on([
    ...
    if (is(hover, 'apmn:Lane') && !isAny(shape, [ 'apmn:Lane', 'apmn:Participant' ])) {
  ...
  eventBus.on([
    ...
    if (
      target && (
        connection.type === 'apmn:Association' ||
        connection.type === 'apmn:DataOutputAssociation' ||
        connection.type === 'apmn:DataInputAssociation' ||
        connection.type === 'apmn:SequenceFlow'
      )
    ) {
      // snap source
    ...
    if (connection.type === 'apmn:MessageFlow') {
  ...
  eventBus.on('resize.start', HIGH_PRIORITY, function(event) {
    ...
    if (is(shape, 'apmn:SubProcess') && isExpanded(shape)) {
    ...
    if (is(shape, 'apmn:Lane') || is(shape, 'apmn:Participant')) {
    ...
    if (is(shape, 'apmn:TextAnnotation')) {
...
BpmnSnapping.prototype.addTargetSnaps = function(snapPoints, shape, target) {
  ...
  if (is(shape, 'apmn:BoundaryEvent') && shape.type !== 'label') {
    target = target.parent;
  }

  // add sequence flow parents as snap targets
  if (is(target, 'apmn:SequenceFlow')) {
  ...
  forEach(siblings, function(sibling) {
    ...
    if (is(sibling, 'apmn:Participant')) {
