...
export default function BpmnCopyPaste(
  ...
  eventBus.on('element.paste', function(context) {
    ...
    properties = filter(properties, function(property) {
      return IGNORED_PROPERTIES.indexOf(property.replace(/apmn:/, '')) === -1;
    });
    ...
    if (is(parent, 'apmn:Process')) {
      descriptor.parent = is(rootElement, 'apmn:Collaboration') ? rootElement : parent;
    }

    if (descriptor.type === 'apmn:DataOutputAssociation' ||
        descriptor.type === 'apmn:DataInputAssociation' ||
        descriptor.type === 'apmn:MessageFlow') {
      descriptor.parent = rootElement;
    }

    if (is(parent, 'apmn:Lane')) {
    ...
    if (descriptor.type === 'apmn:Participant' && descriptor.processRef) {
      descriptor.processRef = newBusinessObject.processRef = bpmnFactory.create('apmn:Process');
    }
