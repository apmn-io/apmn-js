...
/**
 * This module takes care of replacing APMN elements
 */
export default function BpmnReplace(
  ...
  function replaceElement(element, target, hints) {
    ...
    var properties = filter(copyProps, function(property) {
      var propName = property.replace(/apmn:/, '');
    ...
    // initialize custom APMN extensions
    ...
    if (is(oldBusinessObject, 'apmn:Activity')) {
      ...
      // except apmn:Task, because Task is always expanded
      if ((isExpanded(oldBusinessObject) && !is(oldBusinessObject, 'apmn:Task')) && newElement.isExpanded) {
    ...
    if (is(oldBusinessObject, 'apmn:Participant')) {

      // create expanded pool
      if (target.isExpanded === true) {
        newBusinessObject.processRef = bpmnFactory.create('apmn:Process');
      } else {
